...
function buildLocalPredictions(data) {
  ...
  return [
    {
      label: "24 h",
      zone: top[0]?.displayAddress || top[0]?.zone || "Sector 1",
      risk: top[0]?.risk || "medium",
      window: "20:00 - 02:00",
      reason: "Concentració recent de serveis rellevants i pressió territorial detectada."
    },
    {
      label: "48 h",
      zone: top[1]?.displayAddress || top[1]?.zone || "Eix comercial",
...
function generateSipdaAnswer(question) {
  ...
  if (!data) {
    return `<strong>Encara no hi ha dades carregades</strong><p>Importa un informe o espera que es carregui la jornada base.</p>`;
  }
  ...
  if (/reforc|reforç|patrulla|dispositiu|recomana/.test(q)) {
    return `
      ...
      <p>${data.summary?.recommendation || "Reforçar punts calents i franges de major pressió."}</p>
      ${answerPrediction(data)}
    `;
  }

  return `
    <strong>Lectura SIPDA</strong>
    <p>${data.summary?.executiveRead || "Informe carregat sense lectura executiva."}</p>
    <p>Pots demanar: <b>briefing</b>, <b>zones calentes</b>, <b>previsió 48 h</b> o <b>informe executiu</b>.</p>
  `;
}
