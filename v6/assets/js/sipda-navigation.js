...
function bindSipdaNavigation() {
  ...
  const observedSections = ["#top", "#mapa", "#ia", "#prediccio", "#risc", "#docs"]
  ...
  const observer = new IntersectionObserver(
    (entries) => {
      ...
      if (visible && visible.target.id) setActiveNavigation(`#${visible.target.id}`);
    },
    { root: null, threshold: [0.28, 0.45, 0.65], rootMargin: "-14% 0px -58% 0px" }
  );

  observedSections.forEach((section) => observer.observe(section));
  if (location.hash) setTimeout(() => scrollToSection(location.hash), 250);
  else setActiveNavigation("#top");
}
