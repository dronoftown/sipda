...
(function(){
  const BUILD='final-pdf-controller-2026-05-20';
  ...
  function plAddress(block){const f=O(block);let m=f.match(/(?:LOCALITZACI[ÓO]\s+)?Via\s*1\s*:?\s*(.*?)(?=\s+(?:Via\s*2|Descripci[oó]|Descripcio|Not[ií]cia|Noticia|Requeriment|Hora inici|ESTAD|Resultat|Unitat|Dia i hora|N[uú]m\.?\s*Servei|Num\.?\s*Servei)\b|$)/i);if(m&&m[1])return ADR(m[1]);m=f.match(/\b((?:AV|AVDA|AVGDA|C|C\.|CL|CR|CARRER|PL|PÇA|PLAÇA|PG|PS|PASSEIG|CTRA|RBLA|PTGE|TRAV|CAM[IÍ]|URB)\s+[A-ZÀ-ÿ0-9'’.,\- ]{2,120}\s+\d{1,4})\b/i);return m&&m[1]?ADR(m[1]):''}
