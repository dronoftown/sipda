...
(function(){
  ...
  function ensureMeteoCard(){
    ...
    old.outerHTML=`<article id="meteoAlertCard" class="mini-kpi-card meteo-kpi-card" aria-label="Alertes meteorològiques 48 hores"><div class="meteo-compact-top"><div class="meteo-icon"><i data-lucide="cloud-sun"></i></div><div class="meteo-title"><small>Alertes meteorològiques · 48 h</small><strong id="meteoRiskLabel">Verificant fonts</strong><span>Castell d'Aro · Platja d'Aro · S'Agaró</span></div><b id="meteoRiskBadge" class="meteo-badge checking">Oficial</b></div><div class="meteo-compact-grid"><span><i data-lucide="cloud-rain"></i><b id="meteoRain">Pluja --</b></span><span><i data-lucide="wind"></i><b id="meteoWind">Vent --</b></span><span><i data-lucide="waves"></i><b id="meteoSea">Mar --</b></span></div><div class="meteo-source-line"><em>Meteocat · AEMET · Protecció Civil</em><strong id="meteoUpdated">48 h</strong></div></article>`;
  ...
  function updateMeteoCard(){
    ...
    const updated=document.getElementById('meteoUpdated');
    const rain=document.getElementById('meteoRain');
    const wind=document.getElementById('meteoWind');
    const sea=document.getElementById('meteoSea');
    const source=window.SIPDA_METEO_48H||null;
    const now=new Date().toLocaleTimeString('ca-ES',{hour:'2-digit',minute:'2-digit'});
    if(source){
      if(label)label.textContent=source.riskLabel||'Sense alerta oficial';
      if(badge){badge.textContent=source.riskBadge||'48 h';badge.className='meteo-badge '+(source.riskLevel||'low')}
      if(rain)rain.textContent=source.rain||'Pluja --';
      if(wind)wind.textContent=source.wind||'Vent --';
      if(sea)sea.textContent=source.sea||'Mar --';
      if(updated)updated.textContent=source.updated||now;
    }else{
      if(label)label.textContent='Fonts oficials';
      if(badge){badge.textContent='48 h';badge.className='meteo-badge checking'}
      if(rain)rain.textContent='Pluja oficial';
      if(wind)wind.textContent='Vent oficial';
      if(sea)sea.textContent='Mar oficial';
