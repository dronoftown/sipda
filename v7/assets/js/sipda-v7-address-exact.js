...
(function(){
  const BUILD='exact-address-2026-05-20';
  const originalBuild=typeof build==='function'?build:null;
  const names=[[/\bS\s*['’]?\s*AGARO\b/gi,"S'Agaró"],[/\bS\s*['’]?\s*AGARÓ\b/gi,"S'Agaró"],[/\bPLATJA\s+D\s*['’]?\s*ARO\b/gi,"Platja d'Aro"],[/\bCASTELL\s+D\s*['’]?\s*ARO\b/gi,"Castell d'Aro"],[/\bSANTIAGO\s+RUSI[NÑ]OL\b/gi,'Santiago Rusiñol']];
  function L(v){return typeof line==='function'?line(v):String(v||'').replace(/\s+/g,' ').trim();}
  function clean(v){return L(v).replace(/^\s*(?:via\s*1|via|adreça|adreca|ubicació|ubicacio|lloc|lloc\s+detall)\s*:\s*/i,'').replace(/\s+·\s+ubicaci[oó]\s+protegida\s*$/i,'').replace(/\s*,\s*/g,', ').trim();}
  function expand(v){
    v=clean(v);
    ...
    return v.replace(/\bAV\.?\b/gi,'Avinguda').replace(/\bAVDA\.?\b/gi,'Avinguda').replace(/\bCL\.?\b/gi,'Carrer').replace(/\bCTRA\.?\b/gi,'Carretera').replace(/\bPG\.?\b/gi,'Passeig').replace(/\bPS\.?\b/gi,'Passeig').replace(/\bPÇA\b/gi,'Plaça').replace(/\s+/g,' ').replace(/\s+,/g,',').trim();
  }
  ...
  function address(v){v=expand(v);if(!v)return 'Adreça no detectada';if(/^zona\b|^sector\b|^punt\b|^municipi\b/i.test(v))return 'Adreça no detectada';if(hasStreet(v)&&hasNo(v))return v;if(hasStreet(v))return v+' · número no detectat';return hasNo(v)?v:'Adreça no detectada';}
  function rebuild(o){
