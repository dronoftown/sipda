...
(function(){
  ...
  const BUILD='sipda-v7-gem-launcher-exact-tab-2026-05-22';
  ...
  function style(){
    ...
    css.textContent=`
      ...
      .sipda-gem-link{word-break:break-all;font-size:11px;line-height:1.45;color:#475467;background:#fff;border:1px dashed #cbd5e1;border-radius:14px;padding:10px 12px}
  ...
  function mount(){
    ...
    if(!document.getElementById('sipdaGemPanel')){
      ...
      panel.innerHTML=`
        ...
        <div class="sipda-gem-body">
          <div class="sipda-gem-note">SIPDA obre l'enllaç exacte del Gem en una pestanya normal. Si Google mostra un assistent genèric, revisa que el Gem estigui compartit i que el compte de Google tingui accés.</div>
          <div class="sipda-gem-actions">
            <button id="sipdaGemOpenWindow" class="sipda-gem-action primary" type="button"><i data-lucide="external-link"></i>Obrir Gem SIPDA exacte</button>
            <button id="sipdaGemCopy" class="sipda-gem-action" type="button"><i data-lucide="copy"></i>Copiar enllaç Gem</button>
            <a class="sipda-gem-action" href="${GEM_URL}" target="_blank"><i data-lucide="send"></i>Obrir com enllaç directe</a>
          </div>
          <div class="sipda-gem-card">
            <h3>Agent extern vinculat</h3>
            <p>Aquest botó no incrusta Gemini ni modifica l'enllaç: obre exactament el Gem compartit.</p>
            <div class="sipda-gem-list"><span><i></i>Sense iframe i sense popup especial.</span><span><i></i>Obertura en pestanya normal per conservar sessió i permisos de Google.</span><span><i></i>Si apareix genèric, el problema és de permisos/compartició del Gem a Google.</span></div>
          </div>
          <div class="sipda-gem-link">${GEM_URL}</div>
          <div id="sipdaGemStatus" class="sipda-gem-status">Preparat per obrir el Gem.</div>
        </div>`;
      ...
      document.getElementById('sipdaGemCopy')?.addEventListener('click',copyGemLink);
    }
    ...
    window.SIPDA_GEM_PANEL={build:BUILD,url:GEM_URL,open,close,openGemWindow,copyGemLink};
  }
  ...
  async function copyGemLink(){
    const status=document.getElementById('sipdaGemStatus');
    try{
      await navigator.clipboard.writeText(GEM_URL);
      if(status) status.textContent='Enllaç del Gem copiat. Enganxa’l en una pestanya nova si cal.';
    }catch(e){
      if(status) status.textContent='No s’ha pogut copiar automàticament. Copia l’enllaç manualment del requadre superior.';
    }
  }
