...
(function(){
  ...
  function markerLayer(x){
    ...
    const popup=`<div class="map-popup"><div class="map-priority-title">${sChipLocal(x.sourceType)} ${pBadgeLocal(priority)}</div><div class="map-popup-body"><strong>${E(x.title)}</strong><span class="map-popup-zone">${E(x.zone)}</span></div></div>`;
  ...
  if(typeof oldRender==='function'){
    window.render=function(){
      ...
      setTimeout(()=>window.drawMap(true),0);
    };
  }
  document.addEventListener('DOMContentLoaded',()=>{
    setTimeout(()=>{ensureUI();bindInteraction();window.drawMap(true);},650);
    setTimeout(()=>{ensureUI();bindInteraction();window.drawMap(true);},1600);
  });
})();